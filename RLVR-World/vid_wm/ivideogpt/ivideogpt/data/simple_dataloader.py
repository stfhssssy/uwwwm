...
def extract_goal_image(
    ...
    valid_len: Optional[Union[int, Sequence[int], np.ndarray, torch.Tensor]] = None,
):
    """
    Extract goal image (the last valid frame) from trajectory frames.
    ...
    Args:
        frames: trajectory frames as numpy array or torch tensor.
        valid_len: optional valid trajectory length(s). If provided, the goal frame
            index is `valid_len - 1` (clamped to [0, T-1]).
            - unbatched: int
            - batched: int or [B]
    """
    if not isinstance(frames, (np.ndarray, torch.Tensor)):
        raise TypeError(f"`frames` must be np.ndarray or torch.Tensor, got {type(frames)}")
    if frames.ndim < 2:
        raise ValueError(f"`frames` must be [T, ...] or [B, T, ...], got shape={frames.shape}")

    is_torch = isinstance(frames, torch.Tensor)
    is_batched = frames.ndim >= 3
    seq_len = int(frames.shape[1] if is_batched else frames.shape[0])
    if seq_len <= 0:
        raise ValueError("`frames` has empty time dimension.")

    def clamp_index(length_value):
        return max(0, min(int(length_value) - 1, seq_len - 1))

    if not is_batched:
        if valid_len is None:
            idx = seq_len - 1
        else:
            if isinstance(valid_len, (list, tuple, np.ndarray, torch.Tensor)):
                if np.asarray(valid_len).ndim != 0:
                    raise ValueError("Unbatched `frames` expects scalar `valid_len`.")
            idx = clamp_index(valid_len)
        return frames[idx]

    batch_size = int(frames.shape[0])
    if valid_len is None:
        if is_torch:
            idx = torch.full((batch_size,), seq_len - 1, dtype=torch.long, device=frames.device)
        else:
            idx = np.full((batch_size,), seq_len - 1, dtype=np.int64)
    else:
        if is_torch:
            idx = torch.as_tensor(valid_len, dtype=torch.long, device=frames.device)
            if idx.ndim == 0:
                idx = idx.repeat(batch_size)
            if idx.ndim != 1 or idx.shape[0] != batch_size:
                raise ValueError(f"Batched `frames` expects `valid_len` with shape [B], got {tuple(idx.shape)}")
            idx = torch.clamp(idx - 1, min=0, max=seq_len - 1)
        else:
            idx = np.asarray(valid_len, dtype=np.int64)
            if idx.ndim == 0:
                idx = np.repeat(idx, batch_size)
            if idx.ndim != 1 or idx.shape[0] != batch_size:
                raise ValueError(f"Batched `frames` expects `valid_len` with shape [B], got {idx.shape}")
            idx = np.clip(idx - 1, 0, seq_len - 1)

    if is_torch:
        batch_idx = torch.arange(batch_size, device=frames.device)
        return frames[batch_idx, idx]
    batch_idx = np.arange(batch_size)
    return frames[batch_idx, idx]


def extract_initial_image(frames: Union[np.ndarray, torch.Tensor]):
    """
    Extract initial image (the first frame) from trajectory frames.
    ...
    if frames.ndim < 2:
        raise ValueError(f"`frames` must be [T, ...] or [B, T, ...], got shape={frames.shape}")
    if (frames.shape[1] if frames.ndim >= 3 else frames.shape[0]) <= 0:
        raise ValueError("`frames` has empty time dimension.")
    return frames[:, 0] if frames.ndim >= 3 else frames[0]


def extract_goal_image_from_episode(npz_path: str, display_key: str = "image") -> np.ndarray:
    """
    Load one RT1 episode `.npz` and return the goal image (last frame).
    """
    with np.load(npz_path) as episode:
        if display_key not in episode:
            available = list(episode.keys())
            raise KeyError(f"`{display_key}` not found in {npz_path}. Available keys: {available}")
        frames = episode[display_key]
    return extract_goal_image(frames)


def extract_initial_image_from_episode(npz_path: str, display_key: str = "image") -> np.ndarray:
    """
    Load one RT1 episode `.npz` and return the initial image (first frame).
    """
    with np.load(npz_path) as episode:
        if display_key not in episode:
            available = list(episode.keys())
            raise KeyError(f"`{display_key}` not found in {npz_path}. Available keys: {available}")
        frames = episode[display_key]
    return extract_initial_image(frames)
