...
class DrMAgent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
        ...
        self.policy_resize = transforms.Resize(84)
        # models
    ...
    def act(self, obs, step, eval_mode):
        obs = self.policy_resize(torch.as_tensor(obs, device=self.device).unsqueeze(0))
    ...
    def update(self, batch, step):
        ...
        obs, action, reward, discount, next_obs = utils.to_torch(
            batch[:5], self.device)
        obs = self.policy_resize(obs)
        next_obs = self.policy_resize(next_obs)
        # augment
