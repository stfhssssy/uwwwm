...
def plot_eval_model(device_id, model, test_loader, minibatch = 16, save_path = 'img.png'):
    ...
    def draw_plot(mu, sigma):
        ...
        for i in range(1, xaxis_limit):
            ...
            plt.close()
        return data_list
    for sidx, (init_img, goal_img, mid_img, relative_position, delta_goal_init) in enumerate(tqdm.tqdm(test_loader)):
        seq_list = []
        sigma_list = []
        init_img = init_img[None,...].to(device_id)
        goal_img = goal_img[None,...].to(device_id)
        ...
        render_video(compose_img_plot_list(init_img_list, mid_img_list, goal_img_list, plot_img_list), save_path + f'{sidx}', 60)
        if sidx > 10:
            break
    return
...
def img_unnormalize(img, row = 1, col = 1):
    ...
    img = (((img.cpu() * std.reshape(1,3,1,1) + mean.reshape(1,3,1,1)) * 255).cpu().data.numpy()).transpose(0,2,3,1).astype(np.uint8)
...
def main(args):
    ...
    dataset_eval_syn = SegmentFrameloader(
        root_dir='/home/ripl-pc/Desktop/PROGRESSOR/progressor_ds/metaworld_expert_demonstrations',  # TODO: replace with actual path
        train = True,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #eval_model(device_id, model, dataset_eval, minibatch = 16, save_path = args.output_dir + f'/sync_eval_{epoch:07d}.png')
        ...
        #num_of_bins = model.module.num_of_bins
        for idx, (cur_batch, cur_lbl) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
            lr = adjust_learning_rate(optimizer, args, idx + epoch * len(data_loader_train), len(data_loader_train))
            cur_batch, cur_lbl = cur_batch.to(device_id), cur_lbl.to(device_id)
            cur_lbl = (cur_lbl[:,1] - cur_lbl[:,0]) / (cur_lbl[:,2] - cur_lbl[:,0])
            #frame_num = (cur_lbl[:,2] - cur_lbl[:,0])

            optimizer.zero_grad()
            cur_batch = cur_batch.permute(1,0,2,3,4).flatten(0,1)
            img_list = cur_batch.chunk(3, dim = 0)

            pred_dist, feat = model(img_list)
            ...
            pred_neg_dist, _ = model(neg_img_list)
        ...
        # Evaluation loop
        if args.eval_path != '' and (epoch % args.eval_every == 0 or epoch == args.epochs - 1) and epoch != 0:
            if device_id == 0:
        ...
        if epoch % 10 == 0:
            save_checkpoint(model.module, epoch, args.output_dir, optimizer, save_last = True)
