...
class Model(nn.Module):
    ...
    '''
    def __init__(self, latent_dim=512, model_type = 'resnet34', cfg = None):
        super().__init__()
        ...
        self.proj = nn.Sequential(MLP(3 * 128, 512),
                                  MLP(512, latent_dim//2))
    ...
    def compute_reward(self,init_mid_goal_img_stack):
        ...
        #if self.cfg is not None: alpha = self.cfg.reward_alpha  
        #else: 
        alpha = 0.4
        
        pred_reward = pred_reward_dist.mean - alpha * pred_reward_dist.entropy()
        return pred_reward
    ...
    def forward(self, img_list, return_feat = False):
        if True:
            ...
            feat = torch.cat(feat_list, dim = -1) * np.sqrt(256)
        else:
            feat = self.model(img_list)
        #feat = self.model(img_list)
        pred = self.proj(feat)
        mu = self.fc_mu(pred)
        ...
        if self.training or return_feat:
            return distributions.Normal(mu.reshape(-1), std.reshape(-1)), feat
        else:
